...
def api_generate_notes():
    try:
        topic = request.json.get('topic')
        print(f"DEBUG: Generating notes for topic: {topic}")
        
        notes_md = generate_gemini_notes(topic)
        if not notes_md or "(Error:" in notes_md:
             print(f"DEBUG: Gemini notes generation failed or returned error: {notes_md[:100]}...")
        
        filename = f"notes_{uuid.uuid4().hex}.docx"
        filepath = os.path.join(current_app.config['DOCS_FOLDER'], filename)
        
        print(f"DEBUG: Creating DOCX at {filepath}...")
        success = create_docx(notes_md, filepath)
        if not success:
            print("DEBUG: create_docx failed!")
            return jsonify({'success': False, 'message': 'Failed to create document file locally.'}), 500
            
        download_url = url_for('static', filename='documents/'+filename)
        
        if current_app.config.get('SUPABASE_URL'):
            print("DEBUG: Attempting Supabase upload...")
            try:
                with open(filepath, 'rb') as f:
                    supa_url = upload_file_to_supabase(current_app.config.get('SUPABASE_BUCKET', 'dronacharya'), f, f"notes/{filename}")
                    if supa_url: 
                        download_url = supa_url
                        print(f"DEBUG: Supabase upload successful: {supa_url}")
                    else:
                        print("DEBUG: Supabase upload failed, falling back to local URL.")
            except Exception as e:
                print(f"DEBUG: Supabase upload block failed: {e}")
        
        return jsonify({'success': True, 'download_url': download_url})
    except Exception as e:
        import traceback
        print(f"CRITICAL ERROR in api_generate_notes: {e}")
        traceback.print_exc()
